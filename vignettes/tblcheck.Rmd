...
1. Compare student output to model solutions with the [`tbl_grade_table()` function family](#grading-tables), or

1. Write custom grading logic with the [`tbl_check_table()` function family](#custom-mistake-handling).
...
For exercises that use vectors rather than tables, `tbl_grade_vector()` allows you to apply the same tests that are normally applied to the columns of a table. It checks that the user's vector

1. is the correct class
1. is the correct length
1. contains the correct values
1. has the correct names

`tbl_grade_vector()` includes options to disable any of these checks. If a check fails, the student will see a single message with the first detected problem, based on the order described above.

To use `tbl_grade_vector()`, ensure you have a `-solution` chunk and add `tbl_grade_vector()` to the grading code in you `-check` chunk, e.g.
...
````markdown
```{r storm_names-setup, chunk_label = "storm_names-setup"}
library(dplyr)
storm_names <- storms$name
```

```{r storm_names, chunk_label = "storm_names", is_exercise = TRUE}
grep(storm_names, "[[:lower:]]")
```

```{r storm_names-solution, chunk_label = "storm_names-solution"}
grepl(storm_names, "[[:lower:]]")
```

```{r storm_names-check, chunk_label = "storm_names-check", grade_this = TRUE}
tbl_grade_vector()
```
````

```{r echo=FALSE}
fake_grade_chunks("storm_names")
```
...
- `tbl_grade_dimensions()` grades the `length`, `ncol`, or `nrow` of an object.
...
If this is a mistake you expect a lot of students to make, you may want to give it special handling. All `tbl_grade_*()` functions have a `tbl_check_*()` equivalent, allowing you to add custom handling for any detected mistake.

For example, where `tbl_grade_table()` returns a _grade_, `tbl_check_table()` returns a _problem_ that you can inspect in your grading code.
...
You can use `if` statements to give specific feedback based on the nature of the `problem` object using the standard `gradethis` functions. For problems not handled by your custom grading code, you can pass the problem to `tbl_grade()` to create a grade from the problem using the default feedback provided by `tbl_grade_table()`.
