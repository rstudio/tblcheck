...
tblcheck produces automatic feedback for [common problems in tables](#grading-tables) with `tbl_grade()`.
...
`tbl_grade()` compares the result of the user's input to the result of the `-solution` chunk, automatically returning targeted feedback to the user if any problems are discovered.
...
To use `tbl_grade()`, ensure you have a `-solution` chunk and add `tbl_grade()` to the grading code in your `-check` chunk, e.g.
...
If the user's submitted table differs from the correct table, `tbl_grade()` returns a failing grade and a message with an explanation for what went wrong. 
If there are multiple problems with a student's submission, `tbl_grade()` tries to give the most actionable item first.
...
In real-world usage, students are likely to only encounter one or two of the problems `tbl_grade()` is designed to find.

#### Checking class

First, `tbl_grade()` ensures that the class of the student's submission matches the class of the expected solution.
...
Here, `tbl_grade()` notices that the student has an unexpected column named `fruit`.
...
Next, `tbl_grade()` checks that the student has submitted the correct number of rows, and in this case notices that the student has only included one row.
...
Next, `tbl_grade()` checks that each individual column contains the correct type of data.
...
Finally, `tbl_grade()` gives a hint as to what the values in each column should look like.
...
When your exercise uses vectors rather than tables, `vec_grade()` allows you to apply the same tests that are normally applied to the columns of a table to a vector.
...
Like `tbl_grade()`, if a problem is detected by any of these checks, the student will see a single message with the first detected problem, based on the order described above.

### Usage

To use `vec_grade()`, ensure you have a `-solution` chunk and add `vec_grade()` to the grading code in your `-check` chunk, e.g.
...
Note that `vec_grade()` only returns feedback when a problem is detected, so be sure to include `gradethis::pass()` or `gradethis::pass_if_equal()` to ensure that students can get a passing grade.
:::

### Finding problems

If the user's submitted vector differs from the correct vector, `vec_grade()` returns a failing grade and a message with an explanation for what went wrong. If there are multiple problems with a student's submission, `vec_grade()` tries to give the most actionable item first.
...
For example, if the student submits a vector of the wrong class, that will be the first message returned by `vec_grade()`.
...
If the student submits a factor with the wrong factor levels, `vec_grade()` will warn the student about their mistake.
...
- The first is to [enable or disable specific checks](#skipping-tests) using the `check_*` arguments of `tbl_grade()` and `vec_grade()`.
...
Every test performed by `tbl_grade()` or `vec_grade()` can be enabled or disabled with an argument.
...
If you don't care about the class of the table, you can add `check_class = FALSE` to `tbl_grade()`. This will skip checking the table's class, but still run all other tests.

````markdown
```{r skip-check-class-check, chunk_label = "food-check", grade_this = TRUE}
tbl_grade(check_class = FALSE)
pass()
```
````
...
Since the only problem with the student's submission was the class of the table, `tbl_grade()` doesn't directly return any feedback.
Notice that we changed the grading code slightly to `pass()` any answer that also passes the `tbl_grade()` checks.

### Specific grading functions

`tbl_grade()` and `vec_grade()` calls a number of grading functions internally. You can call these functions directly to perform more specific grading.

|Function                                         |Grades                                                         |
|-------------------------------------------------|---------------------------------------------------------------|
|`tbl_grade_class()` `vec_grade_class()`          |the class of an object                                         |
|`tbl_grade_column()`                             |the tests in `vec_grade()` to a single column of a table|
...
If there are no problems, `tblcheck_grade(problem)` won't return anything.
