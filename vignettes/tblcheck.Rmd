...
use `grade_this_table()`, `grade_this_column()` or `grade_this_vector()`.
2. To integrate tblcheck into existing grading code, you can use `tbl_grade()`, `tbl_grade_column()` or `vec_grade()` for specific automated checks.
...
grade_this_table(pre_check = {
  gradethis::fail_if(!inherits(.result, "data.frame"))
