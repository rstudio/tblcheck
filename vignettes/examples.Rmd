...
/* tab button colors */
button[id^="correct"][role="tab"] > svg {
  fill: var(--bs-success);
}
button[id^="wrong"][role="tab"] > svg {
  fill: var(--bs-danger);
}
button[role="tab"] > svg {
  margin-right: 3px;
...
> Use `transmute()` to turn convert `height` into inches and `mass` into pounds, keeping only those columns.
...
and tries to use `order()` to set the order of the factor levels in the `martial` column.
...
> Consider the `fruit` dataset. Use `str_subset()` to create a vector
> containing only the fruits with a space in their names.
...
### `r icons$wrong` Wrong Function (2)
